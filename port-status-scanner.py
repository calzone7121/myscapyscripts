...
def main():
    target_ip, target_string = get_ip()
    port_range, port_string, show_flag = get_port_range()
    while True:
        print("="*6 + "MENU" + "="*6)
        print("Target: " + str(target_string))
        print(port_string)
...
def get_ip():
    while True:
        input_range = input("Enter valid Target IP or Domain (Ex: www.----.com or 10.0.0.1): ")
        if valid_ip_pattern.search(input_range):
            try:
                target_ip = input_range
                target_string = (socket.gethostbyaddr(input_range)[0] + " >> " + str(target_ip))
                return target_ip, target_string
            except:
                target_ip = input_range
                target_string = ("Unknown Hostname" + " >> "+ str(target_ip))
                return target_ip, target_string
        else:
            try:
                target_ip = socket.gethostbyname(input_range)
                target_string = (input_range + " >> " + target_ip)
                return target_ip, target_string
            except:
                print("Error, unable to resolve domain name, try again")
                continue
